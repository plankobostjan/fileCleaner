...
def arguments():
    ...
    parser.add_argument("--database-mode", action="sotore_true", help="Use database for storing data for the program")
    args = parser.parse_args()

    if args.add_endings is not None:
        addEndings(args.add_endings)
        sys.exit()
    ...
    if args.add_folders is not None:
    ...
    if args.walk is not None:
        ...
        walkDirectoryTree(endings, log, args.walk, excluded_floders, excluded_files)
        sys.exit()

    if args.list_folders == True:
        folders = getFolders()
    ...
    if args.remove_folders is not None:
        removeFolders(args.remove_folders)
        sys.exit()

    if args.add_excluded_folders is not None:
        addExcludedFolders(args.add_excluded_folders)
        sys.exit()

    if args.list_excluded_folders:
    ...
    if args.remove_excluded_folders is not None:
        removeExcludedFolders(args.remove_excluded_folders)
        sys.exit()

    if args.add_excluded_files is not None:
        addExcludedFiles(args.add_excluded_files)
        sys.exit()

    if args.list_excluded_files == True:
        excluded_files = getExcludedFiles()
    ...
    if args.remove_excluded_files is not None:
...
def main():
    try:
        ...
        deleteFile(folders, endings, log, excluded_folders, excluded_files)
        log.close()
